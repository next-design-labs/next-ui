...
const AvatarShowcase = () => {
  const avatars = [
    { src: "https://i.pravatar.cc/300?img=26", alt: "User 1", fallback: "U1" },
    { src: "https://i.pravatar.cc/300?img=13", alt: "User 2", fallback: "U2" },
  ];

  const solidAvatars = [
    { variant: "solid", color: "primary", fallback: "V" },
    { variant: "solid", color: "blue", fallback: "BG" },
    { variant: "solid", color: "primary", fallback: "V" },
    { variant: "solid", color: "blue", fallback: "BG" },
  ];

  const outlineAvatars = [
    { variant: "outline", color: "primary", fallback: "V" },
    { variant: "outline", color: "blue", fallback: "BG" },
    { variant: "outline", color: "primary", fallback: "V" },
    { variant: "outline", color: "blue", fallback: "BG" },
  ];
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-4">
        {avatars.map(({ src, alt, fallback }, index) => (
          <NuiAvatar
            key={index}
            src={src}
            alt={alt}
        ...
        {solidAvatars.map(({ variant, color, fallback }, index) => (
          <NuiAvatar
            key={index}
            variant={variant}
            color={color}
      ...
      <div className="flex gap-4">
        ...
        {outlineAvatars.map(({ variant, color, fallback }, index) => (
          <NuiAvatar
            key={index}
            variant={variant}
            color={color}
      ...
      <div className="flex flex-col gap-6 p-6 bg-white rounded-lg border border-slate-200">
        ...
        <div className="flex flex-wrap gap-3">
          {["Seamless", "Flexible", "Open Source", "Accessible"].map(
            (tag, index) => (
              <NuiTag key={index} color={index % 2 === 0 ? "sky" : "indigo"}>
                {tag}
              </NuiTag>
            ),
          )}
