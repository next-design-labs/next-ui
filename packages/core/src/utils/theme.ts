...
export const applyThemeCSSVars = (theme: NuiTheme): void => {
  ...
  const cssVariables = flattenCSSVars({
    colors: {
      ...colors,
      ...modes[colorMode], // Merge current mode's overrides
    },
...
export const applyThemeAttributes = (
  ...
  const activeModeStyles = modes[colorMode];
  ...
  targetElement.setAttribute("color-mode", colorMode as string);

  // Apply inline styles for the theme
  targetElement.style.colorScheme = colorMode as string; // Set the color scheme (e.g., "light" or "dark")
  targetElement.style.fontFamily = tokens.font.body; // Apply the font family
  targetElement.style.backgroundColor = activeModeStyles.background || ""; // Set the page background color
