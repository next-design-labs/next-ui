...
/**
 * Gets the active theme based on the provided theme name and color mode.
 */
...
const generateCSSVarString = (theme: NuiTheme) => {
  const { name, colorMode, colors, modes, tokens, typography } = theme;

  // Create a theme attribute selector
  const themeSelector = `[data-theme="${name}"][data-colorMode="${colorMode}"]`;

  // Flatten the theme object into a list of CSS variables
  const cssVars = flattenCSSVars({
    colors: {
      ...colors,
      ...modes.light,
    },
  ...
  // Generate the CSS string with CSS variables
